...
BXX_TEST(create_view_layer_with_name) {
    BXX_ASSERT_EQUAL(get_context().get_scene().add_view_layer("test_layer").get_name(), "test_layer");
    BXX_ASSERT_EQUAL(get_context().get_scene().add_view_layer("test_layer_2").get_name(), "test_layer_2");
}