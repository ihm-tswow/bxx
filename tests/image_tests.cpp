...
BXX_TEST(image_buffer_by_index)
{
    ...
    BXX_ASSERT_EQUAL(buffer.get_index(0), mathutils::rgba<255>(1, 2, 3, 4));

    buffer.set_index(1, { 5,6,7,8 });
    BXX_ASSERT_EQUAL(buffer.get_index(1), mathutils::rgba<255>(5, 6, 7, 8));
}
...
BXX_TEST(image_buffer_by_coord)
{
    ...
    BXX_ASSERT_EQUAL(buffer.get_coord(0, 0), mathutils::rgba<255>( 1,2,3,4 ));

    buffer.set_coord(1, 1, { 5,6,7,8 });
    BXX_ASSERT_EQUAL(buffer.get_coord(1, 1), mathutils::rgba<255>( 5,6,7,8 ));

    buffer.set_coord(1, 0, { 9,10,11,12 });
    BXX_ASSERT_EQUAL(buffer.get_coord(1, 0), mathutils::rgba<255>( 9,10,11,12 ));

    buffer.set_coord(0, 1, { 13,14,15,16 });
    BXX_ASSERT_EQUAL(buffer.get_coord(0, 1), mathutils::rgba<255>( 13,14,15,16 ));
}
